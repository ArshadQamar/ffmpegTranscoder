...
export default function CreateChannel() {
  ...
  // ABR Profiles - Always start with at least one profile
  ...
  const handleFileUpload = (e) =>{
    ...
    reader.onload = (event)=>{
      try{
        ...
        // Handle ABR profiles - always use ABR
        if (data.abr_profiles && data.abr_profiles.length > 0) {
          ...
          // If no ABR profiles in JSON, create one from main output
          setAbrProfiles([{
            output_type: data.output_type || 'udp',
            output_url: data.output_url || '',
            output_multicast_ip: data.output_multicast_ip || '',
            output_network: data.output_network || '',
            video_bitrate: data.video_bitrate?.toString() || '',
            audio_bitrate: data.audio_bitrate?.toString() || '',
            buffer_size: data.buffer_size?.toString() || '',
            resolution: data.resolution || '',
            service_id: data.service_id?.toString() || '',
            video_pid: data.video_pid?.toString() || '',
            audio_pid: data.audio_pid?.toString() || '',
            pmt_pid: data.pmt_pid?.toString() || '',
            pcr_pid: data.pcr_pid?.toString() || '',
            muxrate: data.muxrate?.toString() || ''
          }]);
        }
  ...
  const handleSubmit = async (e) => {
    ...
    // Base payload with ABR always enabled
    const payload = {
      name,
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="px-6 py-5 border-b border-gray-200 dark:border-gray-700">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Create a new channel with Adaptive Bitrate (ABR) output profiles</p>
          </div>
